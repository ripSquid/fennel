...
(fn into-utf8-codepoints [text] (icollect [_ codepoint (utf8.codes text)] codepoint))
...
(fn turn-into-lines [codepoints]
  ...
  (each [_ codepoint (ipairs codepoints)]
    (if
      (= codepoint 10) ; '/n' = 10
      (do (table.insert lines cur) ; if newline, insert current line into lines and set current line to empty
          (set cur {})
      )
  ...
  (if (> (# cur) 0) (table.insert lines cur)) ; add last line to current lines if not empty
...
(fn try-replace [line ...]
    (match [...]
      [pattern format]
        (case (string.match line pattern)
